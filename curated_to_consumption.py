...
def create_promocode_dim(all_sales_df,session)-> None:
    ...
    #fetch existing product dim records.
    existing_promo_code_dim_df = session.sql("select promo_code promotion_code, country, region from sales_dwh.consumption.promo_code_dim")
...
def main():
    ...
    promo_code_dim_df = session.sql("select promo_code_id_pk,promo_code promotion_code,country, region from sales_dwh.consumption.PROMO_CODE_DIM")
